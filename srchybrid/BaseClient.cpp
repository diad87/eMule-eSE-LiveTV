...
// eSE: delegate uTP layer methods to the socket
CUtpSocket* CUpDownClient::InitUtpSupport()
{
	return socket ? socket->InitUtpSupport() : NULL;
}

CUtpSocket* CUpDownClient::GetUtpLayer()
{
	return socket ? socket->GetUtpLayer() : NULL;
}

bool CUpDownClient::IsEseClient() const
{
	CString mod(m_strModVersion);
	mod.Trim();
	return mod.GetLength() >= 3 && mod.Left(3).CompareNoCase(_T("eSE")) == 0;
}

bool CUpDownClient::SupportsLiveP2P() const
{
	return IsEseClient();
}
...
{
	m_bUtpWritable = bWritable;
	if (bWritable && socket != NULL && socket->IsConnected())
		socket->OnSend(0);
}
...
void CUpDownClient::ResetUtpFlowControl()
{
	...
	m_bUtpQueuePurgeRequested = false;
}

void CUpDownClient::RequestUtpQueuePurge()
{
	m_bUtpQueuePurgeRequested = true;
	if (socket != NULL)
...
void CUpDownClient::SendHelloTypePacket(CSafeMemFile &data)
{
	...
	CTag tagModVersion(CT_MOD_VERSION, _T("eSE/0.70b"));
