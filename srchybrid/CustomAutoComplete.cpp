...
STDMETHODIMP CCustomAutoComplete::Next(ULONG celt, LPOLESTR *rgelt, ULONG *pceltFetched) noexcept
{
	...
	for (i = 0; i < celt; ++i) {
		...
		const CStringW strW(m_asList[m_nCurrentElement]);
		const size_t cchBuf = static_cast<size_t>(strW.GetLength()) + 1;
		rgelt[i] = (LPWSTR)::CoTaskMemAlloc(sizeof(WCHAR) * cchBuf);
		wcsncpy_s(rgelt[i], cchBuf, strW, _TRUNCATE);
