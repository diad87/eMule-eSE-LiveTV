		...
		{ // If we are not sending a packet currently, we will need to find one to send
				...
				&& ret.success)
			{
				...
				if (result == (uint32)SOCKET_ERROR) {
					...
					sent += result;
					sentBytes += result; // BUG-002 FIX: accumulate instead of reset
	...
	if (byConnected == EMS_CONNECTED && IsEncryptionLayerReady() && !IsBusyExtensiveCheck() && maxNumberOfBytesToSend > 0) {
		...
		if (sendbuffer != NULL || !controlpacket_queue.IsEmpty() || (!standardpacket_queue.IsEmpty() && !onlyAllowedToSendControlPacket)) {
			...
			if (m_aBufferSend.GetCount() > 0) {
				...
				// BUG-004 FIX: Snapshot stats before WSASend to allow reversal on failure.
				// Stats were accumulated during buffer preparation (above) but WSASend
				// may fail, in which case the bytes were never actually transmitted.
				SocketSentBytes retSnapshot = ret;
				LONG payloadSnapshot = (LONG)m_actualPayloadSizeSent;
				LONG64 partFileSnapshot = (LONG64)m_numberOfSentBytesPartFile;
				LONG64 completeFileSnapshot = (LONG64)m_numberOfSentBytesCompleteFile;
				...
				if (CEncryptedStreamSocket::SendOv(m_aBufferSend, &m_PendingSendOperation) == 0)
					...
					if (nError != WSA_IO_PENDING) {
						ret.success = false;
						// Revert pre-accumulated stats since bytes were never sent
						ret.sentBytesStandardPackets = retSnapshot.sentBytesStandardPackets;
						ret.sentBytesControlPackets = retSnapshot.sentBytesControlPackets;
						InterlockedExchange((LONG*)&m_actualPayloadSizeSent, payloadSnapshot);
						InterlockedExchange64((LONG64*)&m_numberOfSentBytesPartFile, partFileSnapshot);
						InterlockedExchange64((LONG64*)&m_numberOfSentBytesCompleteFile, completeFileSnapshot);
		...
		if (iter->nType == LAYERCALLBACK_LAYERSPECIFIC) {
			if (iter->pLayer == m_pProxyLayer) {
				...
				case PROXYERROR_NOCONN:
					...
					[[fallthrough]]; // Intentional: both NOCONN and REQUESTFAILED append error details and log
...
//sendLock must be locked by the caller!
	if (m_bPendingSendOv) {
		...
		if (bCancel && CancelIo((HANDLE)GetSocketHandle())) {
			// BUG-003 FIX: Wait for cancelled I/O completion without polling Sleep().
			// WaitForSingleObject provides a hard timeout to avoid indefinite blocking
			// if the overlapped event never signals (e.g. driver bug).
			if (m_PendingSendOperation.hEvent != NULL)
				WaitForSingleObject(m_PendingSendOperation.hEvent, 100);
			DWORD dwTransferred, dwFlags;
			WSAGetOverlappedResult(GetSocketHandle(), &m_PendingSendOperation, &dwTransferred, FALSE, &dwFlags);
		}
