	...
	, notifierenabled()
{
	...
	ircwnd = new CIrcWnd;
	livestreamwnd = new CLiveStreamDlg;
...
BOOL CemuleDlg::OnInitDialog()
{
	...
	DialogCreateIndirect(livestreamwnd, IDD_LIVESTREAM);
...
void CemuleDlg::ProcessED2KLink(LPCTSTR pszData)
{
	try {
		...
		case CED2KLink::kSearch:
			...
			break;
		case CED2KLink::kStream:
			{
				CED2KStreamLink *pStreamLink = pLink->GetStreamLink();
				ASSERT(pStreamLink);
				uchar streamKey[16];
...
void CemuleDlg::OnClose()
{
	...
	// Kill eSE server process if running
...
BOOL CemuleDlg::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	case MP_HM_STATS:
		...
		break;
	case TBBTN_LIVESTREAM:
	case MP_HM_LIVESTREAM:
		SetActiveDialog(livestreamwnd);
		break;
	case TBBTN_ESE:
	case MP_HM_ESE:
		ToggleEseServer();
...
void CemuleDlg::ShowToolPopup(bool toolsonly)
{
	...
	if (!toolsonly) {
		...
		menu.AppendMenu(MF_STRING, MP_HM_LIVESTREAM, GetResString(IDS_EM_LIVESTREAM), _T("LIVESTREAM"));
		menu.AppendMenu(MF_STRING, MP_HM_ESE,        GetResString(IDS_EM_ESE),        _T("ESESERVER"));
...
int CemuleDlg::MapWindowToToolbarButton(CWnd *pWnd) const
{
	...
	if (pWnd == livestreamwnd)
		return TBBTN_LIVESTREAM;
	ASSERT(0);
	return -1;
}
...
void CemuleDlg::CreateToolbarCmdIconMap()
{
	...
	m_mapTbarCmdToIcon[TBBTN_LIVESTREAM] = _T("LiveStream");
	m_mapTbarCmdToIcon[TBBTN_ESE]        = _T("ESESERVER");
}
...
// ---------------------------------------------------------------------------
// eSE Server lifecycle
// ---------------------------------------------------------------------------
bool CemuleDlg::IsEseServerRunning() const
{
	return m_hEseProcess != NULL &&
...
//>>> WiZaRd - handle "UPNP_TIMEOUT" events!
	if (!bWasRefresh && wParam != CUPnPImpl::UPNP_OK) {
		...
		// eSE Fase 2: Señalizar error crítico UPnP para la API JSON
...
void CemuleDlg::UpdateThumbBarButtons(bool initialAddToDlg)
{
	...
	for (int i = TBB_FIRST; i <= TBB_LAST; ++i) {
		...
		if (uid) {
			...
			wcsncpy_s(m_thbButtons[i].szTip, _countof(m_thbButtons[i].szTip), tooltip, _TRUNCATE);
