		...
		if (!m_bInitialized) {
			...
			if (m_hLib != NULL) {
				...
				if (ullVersion >= MAKEDLLVERULL(0, 7, 13, 0)
					&& (thePrefs.GetWindowsVersion() >= _WINVER_VISTA_ && ullVersion < MAKEDLLVERULL(26, 2, 0, 0)
...
wchar_t* ID3_GetStringW(const ID3_Frame *frame, ID3_FieldID fldName)
{
	...
	CStringW wstr(text);
	delete[] text;
	const size_t cchBuf = static_cast<size_t>(wstr.GetLength()) + 1;
	wchar_t *pwsz = new wchar_t[cchBuf];
	wcsncpy_s(pwsz, cchBuf, wstr, _TRUNCATE);
	return pwsz;
}
