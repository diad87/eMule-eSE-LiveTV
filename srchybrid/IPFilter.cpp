...
static bool IsValidIPv4RangeOctets(UINT u1, UINT u2, UINT u3, UINT u4, UINT u5, UINT u6, UINT u7, UINT u8)
{
	return u1 <= 255 && u2 <= 255 && u3 <= 255 && u4 <= 255
		&& u5 <= 255 && u6 <= 255 && u7 <= 255 && u8 <= 255;
}
...
INT_PTR CIPFilter::AddFromFile(LPCTSTR pszFilePath, bool bShowResponse)
{
	...
	if (readFile != NULL) {
		...
		try {
			...
			// BUG-076 FIX: handle MFC exceptions
			TCHAR szErr[256];
			e->GetErrorMessage(szErr, _countof(szErr));
			AddDebugLogLine(false, _T("CException loading IP filters - %s"), szErr);
			e->Delete();
			fclose(readFile);
			throw;
		} catch (const std::exception& e) {
			AddDebugLogLine(false, _T("std::exception loading IP filters - %hs"), e.what());
			fclose(readFile);
			throw;
		} catch (...) {
			// BUG-076 FIX: don't re-throw unknown exceptions
			AddDebugLogLine(false, _T("Unknown exception loading IP filters - %s"), _tcserror(errno));
			fclose(readFile);
		}
		fclose(readFile);
			...
			for (INT_PTR i = 1; i < m_iplist.GetCount(); ++i) {
					...
					|| pCur->start == pPrv->end + 1 && pCur->level == pPrv->level) // adjacent
				{
					if (pCur->start != pPrv->start || pCur->end != pPrv->end) { // don't merge identical entries
						// BUG-075 FIX: use most restrictive level when merging overlapping ranges
...
bool CIPFilter::ParseFilterLine1(const CStringA &sbuffer, uint32 &ip1, uint32 &ip2, uint32 &level, CStringA &desc)
{
	...
	if (iItems < 8)
		return false;
	if (!IsValidIPv4RangeOctets(u1, u2, u3, u4, u5, u6, u7, u8))
		return false;
