...
bool CKnownFile::CreateFromFile(LPCTSTR in_directory, LPCTSTR in_filename, LPVOID pvProgressParam)
{
	...
	for (hashcount = 0; ; ++hashcount) {
		...
		if (pvProgressParam) {
			...
			// BUG-032 FIX: dynamic_cast instead of reinterpret_cast to avoid UB
			CPartFile* pPartFile = dynamic_cast<CPartFile*>(reinterpret_cast<CKnownFile*>(pvProgressParam));
			if (pPartFile != NULL && pPartFile->IsDeleting())
			{
