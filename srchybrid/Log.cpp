...
void AddLogTextV(UINT uFlags, EDebugLogPriority dlpPriority, LPCTSTR pszLine, va_list argptr)
{
	...
	int nResult = _vsntprintf(szLogLine, _countof(szLogLine), pszLine, argptr);
	szLogLine[_countof(szLogLine) - 1] = _T('\0');
	// BUG-077 FIX: detect truncation and append indicator
	if (nResult < 0 || nResult >= (int)_countof(szLogLine)) {
		szLogLine[_countof(szLogLine) - 4] = '.';
		szLogLine[_countof(szLogLine) - 3] = '.';
		szLogLine[_countof(szLogLine) - 2] = '.';
	}
