...
void CPPgConnection::LoadSettings()
{
	if (m_hWnd) {
		...
		// Show "Open Ports" button when Windows Firewall is detected
		// Modern API works on Vista+, legacy ICS on WinXP pre-SP2
		bool bCanShow = (wv >= _WINVER_VISTA_ || (wv == _WINVER_XP_ && !IsRunningXPSP2()));
		GetDlgItem(IDC_OPENPORTS)->ShowWindow(bCanShow ? SW_SHOW : SW_HIDE);
		if (bCanShow) GetDlgItem(IDC_OPENPORTS)->EnableWindow(TRUE);
