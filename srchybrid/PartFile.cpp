	...
	while (newBlockCount < iCount) {
		...
		if (chunksList.IsEmpty()) {
			...
			for (POSITION pos = chunksList.GetHeadPosition(); pos != NULL;) {
				...
				// eSE: For video files, prioritize download order for streaming
				if (IsPreviewableFileType()) {
					if (m_nStreamSeekPart != _UI16_MAX) {
						// Streaming seek mode: prioritize from seek target forward
						if (cur_chunk.part >= m_nStreamSeekPart) {
							cur_chunk.rank = cur_chunk.part - m_nStreamSeekPart; // 0, 1, 2, ...
						} else {
							cur_chunk.rank = 10000 + cur_chunk.part; // Low priority for parts before seek
						}
						// Near-complete parts from seek target onward get absolute priority
						if (critCompletion > 50 && cur_chunk.part >= m_nStreamSeekPart) {
							cur_chunk.rank = 0;
						}
					} else {
						// No seek: strict sequential order from start
						if (cur_chunk.part == partCount - 1) {
							cur_chunk.rank = 1;
						} else {
							cur_chunk.rank = cur_chunk.part;
						}
						if (critCompletion > 50) {
							cur_chunk.rank = 0;
						}
					}
				} else
...
// eSE Phase 2: Read stream data from .part file for HTTP streaming
int CPartFile::ReadStreamData(uint64 offset, BYTE* buffer, uint32 length)
{
	...
	// Check that the requested range is fully downloaded (no gaps)
	if (!IsComplete(offset, end))
		return -1; // Data not available yet

	// Read from the .part file
	CSingleLock lock(&m_FileCompleteMutex, TRUE);
	try {
		m_hpartfile.Seek(offset, CFile::begin);
		UINT nRead = m_hpartfile.Read(buffer, length);
		return (int)nRead;
	} catch (CFileException* ex) {
		ex->Delete();
		return -1;
