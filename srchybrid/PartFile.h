...
public:
	...
	// eSE: Stream reading from .part file
	int		ReadStreamData(uint64 offset, BYTE* buffer, uint32 length);

	// eSE: Streaming seek - prioritize downloading from a specific part
	void	SetStreamSeekPart(uint16 part)				{ m_nStreamSeekPart = part; }
	uint16	GetStreamSeekPart() const					{ return m_nStreamSeekPart; }
	void	ClearStreamSeek()							{ m_nStreamSeekPart = _UI16_MAX; }
