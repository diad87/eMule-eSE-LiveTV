...
// eSE default toolbar: Connect│sep│Kad│Svr│Trans│Search│SharedFiles│Msg│IRC│sep│Stats│Options│Tools│Help│LiveStream│eSE
LPCTSTR const strDefaultToolbar = _T("009901020304050607990809101112" "13");
...
void CPreferences::LoadPreferences()
{
	...
	// Migration: if saved config predates the eSE button (slot 13), append it automatically
	if (m_sToolbarSettings.Find(_T("13")) < 0)
		m_sToolbarSettings += _T("13");
	...
	m_bOpenPortsOnStartUp = ini.GetBool(_T("OpenPortsOnStartUp"), true); // eSE: Try firewall on startup
	...
	// eSE: Migración segura de WebUseUPnP con retrocompatibilidad INI
	// Si la clave ya existía (usuario legacy), heredar su valor para no exponerle.
	// Si es instalación limpia, activar por defecto (true).
	m_bWebUseUPnP = ini.GetBool(_T("WebUseUPnP"), true);

	// SEGURIDAD CRÍTICA: Deshabilitar UPnP forzosamente si no hay contraseña de admin
	if (m_bWebUseUPnP && m_strWebPassword.IsEmpty()) {
		m_bWebUseUPnP = false;
	}

	// eSE: Inicializar estado de error de red (siempre limpio al arrancar)
	m_bUPnPCriticalError = false;
						...
						&& (*pfnSHGetKnownFolderPath)(FOLDERID_ProgramData, 0, NULL, &pszProgramData) == S_OK)
					{
							...
							&& _tcsclen(pszPublicDownloads) < MAX_PATH - 40)
						{
							...
							if (nRegistrySetting == _UI32_MAX) {
								// eSE: ZERO-CONFIG ISOLATION. If a preferences.ini exists in our exe config folder,
								// we ALWAYS prioritize it over any AppData configs to run in 100% portable mode.
								if (bConfigAvailableExecutable)
									m_nCurrentUserDirMode = 2;
								else if (::PathFileExists(strLocalAppData + _T("eMule\\") CONFIGFOLDER _T("preferences.ini")))
