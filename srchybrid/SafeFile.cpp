...
// BUG-072 FIX: Maximum allowed string length to prevent OOM from malformed packets
static const UINT MAX_ED2K_STRING_LEN = 10u * 1024u * 1024u; // 10 MB
...
CString CFileDataIO::ReadString(bool bOptUTF8)
{
	...
	// BUG-072 FIX: Reject absurdly large strings that could cause OOM
	if (uLen > MAX_ED2K_STRING_LEN)
		AfxThrowFileException(CFileException::endOfFile, -1, NULL);
...
CStringW CFileDataIO::ReadStringUTF8()
{
	...
	// BUG-072 FIX: Reject absurdly large strings that could cause OOM
	if (uRawSize > MAX_ED2K_STRING_LEN)
		AfxThrowFileException(CFileException::endOfFile, -1, NULL);
	const UINT uMaxShortRawSize = SHORT_RAW_ED2K_UTF8_STR;
	if (uRawSize <= uMaxShortRawSize) {
		char acRaw[uMaxShortRawSize];
