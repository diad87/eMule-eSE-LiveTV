...
bool CServerList::AddServer(const CServer *pServer, bool bAddTail)
{
	...
	// eSE ZERO-CONFIG: If this is the first server we have learned about (via Kad peer exchange)
	// and we are not yet connected to ed2k, trigger an automatic connection attempt.
	// This implements "Kad-first bootstrap" without needing a pre-seeded server.met.
