...
bool CAddFileThread::ImportParts()
{
	...
	for (INT_PTR i = 0; i < m_PartsToImport.GetSize(); ++i) {
		...
		try {
			...
			LogWarning(LOG_STATUSBAR, _T("Import: Unhandled exception while processing part %u of \"%s\""), (unsigned)partnumber, (LPCTSTR)m_strFilename);
		}
	}
	f.Close();
	delete[] partData;
...
void CSharedFileList::SetOutputCtrl(CSharedFilesCtrl *in_ctrl)
{
	...
	// eSE: Start real-time directory watcher once GUI is ready
...
void CSharedFileList::Process()
{
	...
	// eSE: Process files detected by the real-time directory watcher
	CStringList listPending;
	if (m_directoryWatcher.GetPendingFiles(listPending)) {
		for (POSITION pos = listPending.GetHeadPosition(); pos != NULL;) {
			CString strPath = listPending.GetNext(pos);
			if (CheckAndAddSingleFile(strPath))
				AddDebugLogLine(false, _T("eSE: Auto-shared file detected by watcher: '%s'"), (LPCTSTR)strPath);
		}
	}
}
