...
UINT UploadBandwidthThrottler::RunInternal()
{
	...
	while (m_bRun) {
		...
		// eSE Upload Floor: enforce minimum upload when viewing a live stream
		// This ensures that live stream viewers contribute bandwidth back to the
		// mesh network. Without this floor, peers would be banned for low ratio.
		if (theApp.liveStreamManager && theApp.liveStreamManager->IsViewingLive()) {
			uint32 minUpload = theApp.liveStreamManager->GetMinUploadRequired();
			if (minUpload > 0 && allowedDataRate < minUpload) {
				allowedDataRate = minUpload;
			}
		}
