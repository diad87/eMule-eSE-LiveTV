...
{
	if (!to || tolen <= 0)
		return; // owner may be NULL (from RegisterExpectedPeer hole-punch hints)
	...
	for (size_t i = 0; i < g_expectedPeers.size(); ++i) {
		if (SockaddrEqualIPAndPort((sockaddr*)&g_expectedPeers[i].addr, g_expectedPeers[i].len, to, tolen)) {
			// Already exists - refresh TTL. Only upgrade owner (don't degrade non-NULL to NULL).
...
static uint64 on_utp_state_change(utp_callback_arguments* a)
{
	...
	case UTP_STATE_WRITABLE:
		...
		if (natClient) {
			...
			if (false)
				DebugLog(_T("[NatTraversal][uTP] WRITABLE state confirmed for %s"), (LPCTSTR)(natClient->DbgGetClientInfo()));
		}
		break;
	case UTP_STATE_EOF:
...
static uint64 on_utp_sendto(utp_callback_arguments* a)
{
	...
	if (false && thePrefs.GetVerboseLogPriority() <= DLP_VERYLOW)
...
static uint64 on_utp_error(utp_callback_arguments* a)
{
	...
	if (bTransientNatError) {
		...
		bool bNeedNatRetry = (natClient != NULL &&
		...
		if (natClient != NULL) {
			...
			if (bNeedNatRetry && bRetryAllowed) {
				...
				if (bHaveRetryEndpoint) {
					natClient->QueueDeferredNatConnect(retryIP, retryPort, 120);
		...
		if (false)
			DebugLog(_T("[NatTraversal][uTP] ERROR transient: kept client alive for retry (code=%d mappedWSA=%d rearm=%d endpoint=%s:%u)"),
				a->error_code, Error, bRetryScheduled ? 1 : 0, (LPCTSTR)"IP", (UINT)retryPort);
		return 0;
	}
...
static uint64 on_utp_accept(utp_callback_arguments* a)
{
	...
	if (a && a->socket) {
		...
		if (active >= kMaxUtpSockets) {
			if (thePrefs.GetVerbose())
				DebugLog(_T("[NatTraversal][uTP] on_utp_accept: too many active sockets (%u/%u), rejecting"), (UINT)active, (UINT)kMaxUtpSockets);
			utp_close(a->socket);
			return 0;
		}
		...
		if (utp_getpeername(a->socket, (sockaddr*)&peer, &peerlen) == 0) {
				...
				if (existing) {
					...
					if (existing->GetPeerName((sockaddr*)&existingPeer, &existingPeerLen)) {
						...
						if (SockaddrEqualIPAndPort((sockaddr*)&existingPeer, existingPeerLen, (sockaddr*)&peer, peerlen)) {
							...
							uint32 myPublicIPNet = theApp.GetPublicIP(); // network byte order, 0 if unknown
							...
							if (!bCmpReady) {
								...
								if (localHash != NULL && remoteHash != NULL && !isnulmd4(localHash) && !isnulmd4(remoteHash) && !md4equ(localHash, remoteHash)) {
									...
									bCmpReady = true;
									if (thePrefs.GetVerbose())
										DebugLog(_T("[NatTraversal][uTP] Tie-breaking fallback by UserHash (cmp=%d), %s"), cmp, adoptClient ? (LPCTSTR)(adoptClient->DbgGetClientInfo()) : _T("no-owner"));
								}
							}
			...
			for (size_t i = 0; i < g_expectedPeers.size();) {
				...
				if (!adoptOwner && SockaddrEqual((sockaddr*)&g_expectedPeers[i].addr, g_expectedPeers[i].len, (sockaddr*)&peer, peerlen)) {
					if (g_expectedPeers[i].owner != NULL) {
						// Exact match with valid owner — consume and adopt
						adoptOwner = g_expectedPeers[i].owner;
						g_expectedPeers.erase(g_expectedPeers.begin() + i);
						break;
					}
					// NULL owner (from RegisterExpectedPeer): don't consume,
					// let the NatExpectation fallback path handle it.
				}
				++i;
			}
			g_expectedPeersLock.Unlock();
	...
	for (size_t i = 0; i < g_expectedPeers.size();) {
		...
		if (SockaddrEqualIPOnly((sockaddr*)&g_expectedPeers[i].addr, g_expectedPeers[i].len, (sockaddr*)&peer, peerlen)) {
			++ipOwnerCount;
			ipOwner = g_expectedPeers[i].owner; // last owner for this IP (may be NULL from RegisterExpectedPeer)
			// Only consider non-NULL owners for direct adoption — NULL entries
			// come from RegisterExpectedPeer (hole-punch hints) and are handled
			// by the NatExpectation fallback path below (L913+).
			if (ipOwner != NULL) {
				uint16 expPort = 0;
	...
	g_expectedPeersLock.Unlock();
		...
		if (bestOwner && ipOwnerCount == 1) {
			...
			if (bestOwner->GetPeerName((SOCKADDR*)&tmp, &tmplen)) {
				...
				const uint16 kPortWindow = 120; // Accept if peer port is within configured window.
		...
		} else if (bestOwner && ipOwnerCount > 1 && thePrefs.GetVerbose()) {
		...
		if (theApp.clientudp != NULL) {
			...
			if (bHasExpectation) {
				...
				if (utpLayer == NULL)
					utpLayer = pNewSocket->InitUtpSupport();

				if (utpLayer != NULL) {
					// Adopt the incoming connection to this socket layer
					utpLayer->Setup(a->socket, true);
					if (thePrefs.GetVerbose())
						DebugLog(_T("[NatTraversal][uTP] ACCEPT via NatExpectation: adopted incoming connection"));
...
static uint64 on_utp_get_udp_mtu(utp_callback_arguments* a)
{
	...
	if (baseMtu == 0) {
		family = CAddress().IsNull() ? AF_INET : AF_INET6;
    ...
    for (size_t i = 0; i < sockets.size(); ++i) {
		...
		if (cursocket->m_Socket && !writeSnapshot.empty()) {
			...
			if (written > 0) {
				...
				// don't busy-wait here; let timer callers try again later
				continue;
			}
		}
		...
		// eSE: NAT Keepalive Heartbeat
		// CG-NAT (4G/5G carriers) expire UDP state-table entries after 30-60s
		// of inactivity. Sending a zero-length utp_write every 15s forces libutp
		// to emit a protocol-level STATE/ACK frame, keeping the NAT pinhole open
		// without any application payload overhead.
		{
			const DWORD dwNow = ::GetTickCount();
			const DWORD dwElapsed = dwNow - cursocket->m_dwLastKeepAlive;
			if (cursocket->m_Socket &&
				cursocket->m_nLayerState == connected &&
				dwElapsed >= 15000) {
				// Zero-length write: libutp treats this as a no-op for data but
				// still generates a protocol frame if no other traffic is pending.
				utp_write(cursocket->m_Socket, NULL, 0);
				cursocket->m_dwLastKeepAlive = dwNow;
			}
		}
    }

	// Global context tick: ensure libutp processes retransmission timeouts, SYN retries,
	// and deferred ACKs regardless of whether any socket has pending write data.
	// This is critical for connection establishment (SYN-ACK handshake) and keepalives.
	utp_context* ctx = theApp.clientudp ? (utp_context*)theApp.clientudp->GetUtpContext() : NULL;
	if (ctx) {
...
bool CUtpSocket::ProcessUtpPacket(const byte* data, size_t len, const struct sockaddr* from, socklen_t fromlen)
{
  ...
  utp_context* ctx = m_Context ? m_Context : (theApp.clientudp ? ((utp_context*)theApp.clientudp->GetUtpContext()) : NULL);
  if (!ctx)
...
/* static */
bool CUtpSocket::FeedRawUdp(const byte* data, int len, const struct sockaddr* from, int fromlen)
{
	if (len <= 0 || !data || !from)
		return false;

	// Obtain the global uTP context from CClientUDPSocket.
	// If uTP is not initialized yet (no context), there's nothing to process.
	utp_context* ctx = theApp.clientudp ? (utp_context*)theApp.clientudp->GetUtpContext() : NULL;
	if (!ctx)
		return false;
	...
	// utp_process_udp returns 1 if the packet was recognized as uTP, 0 otherwise.
	// This allows us to try uTP first and fall through to eMule protocol handling if not.
	int consumed = utp_process_udp(ctx, data, (size_t)len, from, (socklen_t)fromlen);
	if (consumed && thePrefs.GetVerboseLogPriority() <= DLP_LOW) {
		uint16 srcPort = 0;
		if (from->sa_family == AF_INET)
			srcPort = ntohs(reinterpret_cast<const sockaddr_in*>(from)->sin_port);
		AddDebugLogLine(DLP_LOW, false, _T("[NAT-T][uTP] FeedRawUdp consumed %d bytes from port %u"), len, (UINT)srcPort);
	}
	return consumed != 0;
}
...
void CUtpSocket::ExpectPeer(const struct sockaddr* to, socklen_t tolen)
{
	...
	if (!m_Context && theApp.clientudp)
		m_Context = ((utp_context*)theApp.clientudp->GetUtpContext());
    if (!m_Context)
        return;
...
BOOL CUtpSocket::Connect(const LPSOCKADDR lpSockAddr, int nSockAddrLen)
{
	...
	if (m_nLayerState == notsock || m_nLayerState == unconnected) {
		...
		if (false)
			DebugLog(_T("[NatTraversal][uTP] CONNECT attempt -> %s:%u"), (LPCTSTR)"IP", (UINT)destPort);
		...
		if (ret == 0) {
			...
			if (false)
				DebugLog(_T("[NatTraversal][uTP] CONNECT started -> %s:%u"), (LPCTSTR)"IP", (UINT)destPort);
			return TRUE;
		}
		...
		if (false)
			DebugLogError(_T("[NatTraversal][uTP] CONNECT failed ret=%d err=%lu -> %s:%u"), ret, wsaErr, (LPCTSTR)"IP", (UINT)destPort);
...
CUpDownClient* CUtpSocket::GetOwnerClient() const
{
	// Dynamic resolution: ownerSocket->client is authoritative because the Hello handler
	// may create a CUpDownClient after Setup() already cached m_pOwnerClient as NULL.
	// This fixes flow control (SetUtpWritable, ResetUtpFlowControl) for bare accepts.
	CClientReqSocket* ownerSocket = dynamic_cast<CClientReqSocket*>(m_pOwnerSocket);
	if (ownerSocket && ownerSocket->client)
		return ownerSocket->client;
	return m_pOwnerClient; // fallback to cached value
}
...
void CUtpSocket::Setup(utp_socket* Socket, bool bAcceptedSocket)
{
	...
	if (ownerSocket) {
		...
		if (cryptState == ECS_UNKNOWN || cryptState == ECS_NONE) {
			...
			if (false && m_pOwnerClient != NULL) {
				DebugLog(_T("[NatTraversal][uTP][Setup] Reset encryption state during adoption (state=%u), %s"),
					(unsigned)cryptState, (LPCTSTR)(m_pOwnerClient->DbgGetClientInfo()));
			}
		}
	...
	// eSE: Per-socket streaming delay override.
	// Context-level default is set in CClientUDPSocket::Create(), but accepted
	// sockets may have been created before the context option was applied.
	// Explicitly set per-socket to guarantee all uTP connections (both outgoing
	// and NAT-traversed inbound) use the 300ms target for media streaming.
	utp_setsockopt(m_Socket, UTP_TARGET_DELAY, 300000); // 300ms in µs

	m_ShutDown = 0;
	m_dwLastKeepAlive = ::GetTickCount(); // Initialize NAT keepalive timer

	// Use single global UDP context managed by CClientUDPSocket
	m_Context = theApp.clientudp ? ((utp_context*)theApp.clientudp->GetUtpContext()) : NULL;
...
void CUtpSocket::Destroy()
{
    ...
    // Remove stale expected peer entries before removing from active set,
    // otherwise TTL-only cleanup could match new connections to this destroyed socket.
    RemoveExpectedPeersForOwner(this);
