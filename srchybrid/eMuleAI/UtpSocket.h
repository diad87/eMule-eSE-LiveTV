...
class CUtpSocket : public CAsyncSocketExLayer
{
  public:
	...
	static bool FeedRawUdp(const byte* data, int len, const struct sockaddr* from, int fromlen);
	...
	// Register an expected peer without a CUtpSocket instance (used by hole-punch signaling)
	static void RegisterExpectedPeer(const struct sockaddr* to, socklen_t tolen);
	...
	*/
	...
	CUpDownClient* GetOwnerClient() const;  // Dynamic: resolves through ownerSocket->client
	virtual int Receive(void* lpBuf, int nBufLen, int nFlags = 0);
	virtual int Send(const void* lpBuf, int nBufLen, int nFlags = 0);
	virtual BOOL ShutDown(int nHow = 1);
  ...
  protected:
	...
	DWORD				m_dwLastKeepAlive;		// Tick count of last NAT keepalive heartbeat
